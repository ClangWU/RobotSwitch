...
% 绘制 pos 数据
plot(frc.time, frc.pos(:, 1), 'b-', 'LineWidth', 1); % posy 使用蓝色实线
plot(frc.time, frc.pos(:, 2), 'b--', 'LineWidth', 1); % posz 使用蓝色虚线

% 绘制 posd 数据
plot(frc.time, frc.posd(:, 1), 'r-', 'LineWidth', 1); % posdy 使用红色实线
plot(frc.time, frc.posd(:, 2), 'r--', 'LineWidth', 1); % posdz 使用红色虚线

legend('posy', 'posz', 'posdy', 'posdz'); % 添加图例
xlabel('Time'); % x轴标签
