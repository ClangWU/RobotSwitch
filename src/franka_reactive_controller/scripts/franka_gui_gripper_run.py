...
def open():
	node_process = Popen(shlex.split('rosrun franka_reactive_controller franka_gripper_run_node 1'))
	# messagebox.showinfo("Open Gripper", "Gripper Opened")
	time.sleep(1) # Sleep for 1 seconds
	node_process.terminate()

def close():
	node_process = Popen(shlex.split('rosrun franka_reactive_controller franka_gripper_run_node 0'))
