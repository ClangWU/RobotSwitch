...
int main(int argc, char** argv) 
{
  ...
  ros::Publisher pub = 
  nh.advertise<geometry_msgs::Twist>("/right_velocity", 10); // 创建一个发布器，话题名称是"force_data"
  ...
  while(ros::ok())
  {
    if(right.port.isOpen()){
      ...
      geometry_msgs::Twist rs_msg;
      rs_msg.linear.x = right.right_data._x; 
      rs_msg.linear.y = right.right_data._z; 
      pub.publish(rs_msg); 
    }
    ros::spinOnce(); 
    loop_rate.sleep();
